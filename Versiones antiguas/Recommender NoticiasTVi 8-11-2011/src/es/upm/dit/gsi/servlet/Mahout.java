...
public class Mahout extends HttpServlet {
   ...
   /**
		...
		try {
			...
			if (userId !=null){
				...
				for (int i = 0; i < recommendations.size(); i++) {
					...
					String object = Items.getNameOfItem(itemId);
					out.print(object);
					out.print(":");
	...
	/*
	/**
	 * Genera la estimación del grado de satisfacción que obtendrá el cliente por el visionado
	 * de los contenidos disponibles y que no haya aún
	 * 
	 * @param req,HttpServletRequest
	 * @param res,HttpServletResponse
	 * @throws ServletException si se produce algún error
	 * @throws IOException si se produce algún error
	 */
	/*private void getEstimatedPreference(HttpServletRequest req, HttpServletResponse res)
			throws ServletException, IOException {
		Vector<Long> setOfItems = Items.getItemsIds();
		try {
			PrintWriter out =res.getWriter();
			String callback = req.getParameter(Constants.CALLBACK);
			String identifier = req.getParameter(Constants.IDENTIFIER);
			...
			if (userId !=null){
				...
				for (int i=0; i<setOfItems.size();i++){
					long itemId = setOfItems.elementAt(i);
					if (Preference.userHaveItem(userId, itemId) == false ){
						out.print(Items.getNameOfItem(itemId));
						out.print(":");
						float estimation = recommender.estimatePreference(userId, itemId);
						if (Float.isNaN(estimation)) 	// Nota si obtenemos un valor no válido le metemos un valor intermedio
							estimation = 2.5f;			// REVISARLO EN UN FUTURO			
						out.print(estimation);
						if (i+1 != setOfItems.size())
							out.print(",");
					}
				}
				out.print("})");
			} else {
				LOGGER.warning("Imposible dar preferencias debido a la inexistencia del usuario"); 
			}
		} catch (Exception e) {
			e.printStackTrace();
			res.setStatus(HttpServletResponse.SC_SERVICE_UNAVAILABLE);
   ...
   protected void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		//} else if (req.getParameter("action").equals("getEstimatedPreference")) {
		//	getEstimatedPreference(req, res);
