...
import es.upm.dit.gsi.jdbc.*;
import es.upm.dit.gsi.auxiliar.*;
...
public class NoticiasTVi extends HttpServlet {
	...
	private static Logger LOGGER = Logger.getLogger("servlet.Mahout");
			...
			if (userId !=null){
				...
				for (int i = 0; i < recommendations.size(); i++) {
					...
					Noticia not = new Noticia(Long.toString(contentId),
							...
							Contents.getDateOfContentString(contentId),
	...
	private void getRecommendationSocial(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		try {
			...
			if (userId !=null){
				...
				for (int i=contentsids.size()-1; i>=0; i--){
				...
				for (int m = 0; m < results ; m++) {
					...
					Noticia not = new Noticia(Long.toString(id[m]),
							...
							Contents.getDateOfContentString(id[m]),
		...
		try {
			...
			for (int i=0; i<idsContents.length; i++){
				for (int k=0; k<popular; k++){
					if (Preference.numVoteOfContent((Long)idsContents[i]) >= Constants.MIN_NUM_VOTE){
						...
						if((averageRatings.get((Long)idsContents[i])) != null){
							rate = averageRatings.get((Long)idsContents[i]);
							rate = Fechas.ponderacionTiempo((Long)idsContents[i], rate);
							if (rate>value[k]){
								for (int h=popular-1; h>k; h--){
									id[h]=id[h-1];
									value[h]=value[h-1];
			...
			for (int j=0; j<popular; j++){
				...
				Noticia not = new Noticia(Long.toString(id[j]),
						...
						Contents.getDateOfContentString(id[j]),
		...
		try{
			...
			for (int k=0; k<ids.length; k++){
				...
				Noticia not = new Noticia(Long.toString(ids[k]),
						...
						Contents.getDateOfContentString(ids[k]),
	...
	private void getFavorites(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		try{
			...
			if (userId != null){
				...
				for (int i=preferencesUser.length()-1; i>=0; i--){
					have = "1";
					...
					Noticia not = new Noticia(Long.toString(Contents.getContentId(content)),
							...
							Contents.getDateOfContentString(preferencesUser.getItemID(i)),
	...
	private void getRandom (HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException{
		...
		try{
			...
			for (int k=0; k<Constants.NUM_RESULTS; k++){
				contentId = (long)(rnd.nextDouble()*num+(num-50));
